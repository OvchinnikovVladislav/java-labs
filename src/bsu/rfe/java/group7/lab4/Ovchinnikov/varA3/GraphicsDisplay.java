package bsu.rfe.java.group7.lab4.Ovchinnikov.varA3;
...
// Отобразить график
    ...
    private boolean isIncreasingSequence(double value) {
        ...
        int inex = 0;
        while (!numStr.substring(inex, inex + 1).equals(".")) {
            inex++;
        }
        numStr = numStr.substring(0, inex) + numStr.substring(inex+1);
    ...
    protected void paintMarkers(Graphics2D canvas) {
        ...
        for (Double[] point : graphicsData) {
            double intValue = point[1];
            if (isIncreasingSequence(intValue)) {  // добавлен вызов метода для проверки последовательности цифр
                canvas.setColor(Color.GREEN);  // зеленый цвет для точек, где выполняется условие
            } else {
                canvas.setColor(Color.RED);
            }
            Point2D.Double center = xyToPoint(point[0], point[1]);
            Line2D.Double firstDiagonal = new Line2D.Double();
            firstDiagonal.setLine(

                    constructPoint(xyToPoint(point[0], point[1]), -10, -10),
                    constructPoint(xyToPoint(point[0], point[1]), 10, 10)
            );

            Line2D.Double secondDiagonal = new Line2D.Double();
            secondDiagonal.setLine(
                    constructPoint(xyToPoint(point[0], point[1]), -10, 10),
                    constructPoint(xyToPoint(point[0], point[1]), 10, -10)
            );

            Line2D.Double firstSize = new Line2D.Double();
            firstSize.setLine(
            ...
            Line2D.Double secondSize = new Line2D.Double();
            secondSize.setLine(
                    constructPoint(xyToPoint(point[0], point[1]), 10, -10),
                    constructPoint(xyToPoint(point[0], point[1]), -10, -10)
            );

            Line2D.Double thirdSize = new Line2D.Double();
            thirdSize.setLine(
            ...
            Line2D.Double fourthSize = new Line2D.Double();
            fourthSize.setLine(
                    constructPoint(xyToPoint(point[0], point[1]), -10, 10),
                    constructPoint(xyToPoint(point[0], point[1]), 10, 10)
            );

            canvas.draw(secondDiagonal);
            canvas.draw(firstDiagonal);
            canvas.draw(firstSize);
            canvas.draw(secondSize);
            canvas.draw(thirdSize);
            canvas.draw(fourthSize);
        }
    }
...
// Создать объект контекста отображения текста - для получения характеристик устройства (экрана)
        ...
        if (minX <= 0.0 && maxX >= 0.0 && minY <= 0.0 && maxY >= 0.0) {
            ...
            float x = (float) (origin.getX() + bounds.getWidth());
